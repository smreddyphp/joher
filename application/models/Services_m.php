<?php 
	...
	class Services_m extends CI_Model 
	{
		...
		public function get_services($sid='',$type=""){
			if($sid){
				return $this->db->select('*')->from('services')->where('id',$id)->get()->row_array();
			}else{
				return $this->db->select('*')->from('services')->where('type',$type)->where('status','active')->get()->result_array();
			}
		}
        ...
        * COS( RADIANS( '".$latitude."' )) * COS( RADIANS( `longitude` ) - RADIANS( '".$longitude."' )) ) * 6380 AS `distance`")->from('users')->where('auth_level',$vendor_type)->order_by('distance')->get()->result_array();
		...
		public function get_servicename($vendor_id){
			return $this->db->select('min(vs.price) as price, vs.service_id, s.service_name_en, s.service_name_ar')->from('vendor_services as vs')->join('services as s','vs.service_id = s.id','LEFT')->where('vs.user_id',$vendor_id)->get()->row_array();
			
		}
		//if filter is there
		public function get_filter_vendors($filter,$data){
			$this->db->select("users.user_name,users.user_id,users.email,users.city,users.country,users.address,users.state,users.pincode,users.image,users.shop_name,users.shop_image,users.auth_level,users.latitude,users.longitude,vs.price, vs.service_id, s.service_name_en, ACOS( SIN( RADIANS( `latitude` ) ) * SIN( RADIANS( '".$data['latitude']."' ) ) + COS( RADIANS( `latitude` ) )
        * COS( RADIANS( '".$data['latitude']."' )) * COS( RADIANS( `longitude` ) - RADIANS( '".$data['longitude']."' )) ) * 6380 AS distance")->from('vendor_services as vs')->join('services as s','vs.service_id = s.id','LEFT')->join('users','vs.user_id=users.user_id','LEFT')->where('users.auth_level',$data['vendor_type']);
			...
			if(isset($filter['distance'])){
				return $this->db->where("ACOS( SIN( RADIANS( `latitude` ) ) * SIN( RADIANS( '".$data['latitude']."' ) ) + COS( RADIANS( `latitude` ) )
        * COS( RADIANS( '".$data['latitude']."' )) * COS( RADIANS( `longitude` ) - RADIANS( '".$data['longitude']."' )) ) * 6380 <",$filter['distance'])->get()->result_array();
			}
		...
		public function get_vendor_brands($vendor_id){
			return $this->db->select('vb.*, b.brand_name_en,b.brand_name_ar')->from('vendor_brands as vb')->join('brands as b','vb.brand_id=b.id')->where('vb.user_id',$vendor_id)->get()->result_array();
		}
		//get venodr timings
		...
		public function vender_chats($vendor_id){
			//return $this->db->select('chats.*,users.user_id,user_name,image')->from('chats')->join('users','chats.sender_id=users.user_id','LEFT')->WHERE('chats.receiver_id',$vendor_id)->group_by('chats.sender_id')->order_by('chats.id ASC')->get()->result_array();
			//return $this->db->select('max(chats.id) as id, chats.sender_id, chats.receiver_id,chats.date,,chats.message,users.user_id,user_name,image')->from('chats')->join('users','chats.sender_id=users.user_id','LEFT')->WHERE('chats.receiver_id',$vendor_id)->group_by('chats.sender_id')->order_by('id DESC')->get()->result_array();
			return $this->db->select('c.id, c.sender_id,c.receiver_id, c.message,u.user_name,u.user_id,u.image
FROM chats as c left join users as u on u.user_id=c.sender_id
WHERE c.id IN (
    SELECT MAX(cc.id)
    FROM chats as cc
    GROUP BY cc.sender_id
) and c.receiver_id='.$vendor_id.' ORDER BY c.id DESC')->get()->result_array();
		}
    ...
    public function add_job($data){
    	$price  = $this->get_jobprice($data['services'],$data['vendor_id'])['price'];
    ...
    public function get_user_jobs($user_id){
    	return $this->db->select('jobs.*,users.user_name as vendor_name, users.shop_name')->from('jobs')->join('users','jobs.vendor_id=users.user_id','LEFT')->where('jobs.user_id',$user_id)->where('jobs.status!=',3)->order_by('job_id desc')->get()->result_array();
    }
    public function get_user_bids($user_id){
    	return $this->db->where('user_id',$user_id)->order_by('b_id desc')->get('bids')->result_array();
    }
    ...
    //get services names
    public function get_job_services($services,$lang=""){
        if($lang=='ar'){
           return $this->db->select('GROUP_CONCAT(service_name_ar) as services')->where_in('id',$services,FALSE)->get('services')->row_array(); 
        }
    	else{
    ...
    public function get_allbids(){
        return $this->db->select("bids.*,users.user_name")->join('users','bids.user_id=users.user_id','LEFT')->where('bids.status',0)->order_by('bids.b_id desc')->get('bids')->result_array();
    }
    //for home screen change
    public function get_allserviceproviders($data){
        //return $this->db->select("users.*,ACOS( SIN( RADIANS( `latitude` ) ) * SIN( RADIANS( '".$data['latitude']."' ) ) + COS( RADIANS( `latitude` ) ) * COS( RADIANS( '".$data['latitude']."' )) * COS( RADIANS( `longitude` ) - RADIANS( '".$data['longitude']."' )) ) * 6380 AS `distance`")->from('users')->where_in('auth_level','2,3,4', FALSE)->order_by('distance')->get()->result_array();
		$this->db->select("users.*,ACOS( SIN( RADIANS( `latitude` ) ) * SIN( RADIANS( '".$data['latitude']."' ) ) + COS( RADIANS( `latitude` ) ) * COS( RADIANS( '".$data['latitude']."' )) * COS( RADIANS( `longitude` ) - RADIANS( '".$data['longitude']."' )) ) * 6380 AS `distance`")->from('users');
		if($data['distance']!=""){
			return $this->db->where("ACOS( SIN( RADIANS( `latitude` ) ) * SIN( RADIANS( '".$data['latitude']."' ) ) + COS( RADIANS( `latitude` ) )
        * COS( RADIANS( '".$data['latitude']."' )) * COS( RADIANS( `longitude` ) - RADIANS( '".$data['longitude']."' )) ) * 6380 <",$data['distance'])->where_in('auth_level','2,3,4', FALSE)->get()->result_array();
		//return $this->db->last_query();
		}else{
			return $this->db->where_in('auth_level','2,3,4', FALSE)->order_by('distance')->get()->result_array();
		}	
    }
    // for bankd etails 
	...
	public function update_location($data){
		$user_id = $data['user_id'];
		unset($data['user_id']);
    	$this->db->set($data)->where('user_id',$user_id)->update('users');
    }
	//get brands
	public function get_brands($id=""){
		if($id){
			return $this->db->select('*')->from('brands')->where('id',$id)->get()->row_array();
		}else{
			return $this->db->select('*')->from('brands')->where('status','active')->get()->result_array();
		}
	}
	// book towing
	public function book_towing($data){
		$this->db->insert('towings',$data);
		return $this->db->insert_id();
	}
	//get nearest towers
	public function get_towers($data){
		return $this->db->select("users.user_id, users.latitude, users.longitude,users.device_type,users.device_token,vs.service_id, ACOS( SIN( RADIANS( `latitude` ) ) * SIN( RADIANS( '".$data['latitude']."' ) ) + COS( RADIANS( `latitude` ) )
        * COS( RADIANS( '".$data['latitude']."' )) * COS( RADIANS( `longitude` ) - RADIANS( '".$data['longitude']."' )) ) * 6380 AS distance")->join('vendor_services as vs','users.user_id = vs.user_id','LEFT')->where('auth_level',2)->where('vs.service_id',21)->where("ACOS( SIN( RADIANS( `latitude` ) ) * SIN( RADIANS( '".$data['latitude']."' ) ) + COS( RADIANS( `latitude` ) )
        * COS( RADIANS( '".$data['latitude']."' )) * COS( RADIANS( `longitude` ) - RADIANS( '".$data['longitude']."' )) ) * 6380 <",15)->get('users')->result_array();
		//return $this->db->last_query();
	}
	//get data default
	public function get($table,$colum,$id){
		if($colum!="" && $id!=""){
			return $this->db->where($colum,$id)->get($table)->row_array();
		}else{
			return $this->db->get($table)->result_array();
		}
	}
	//get multiple 
	public function getm($table,$colum,$id){
		if($colum!="" && $id!=""){
			return $this->db->where($colum,$id)->order_by('booked_date desc')->get($table)->result_array();
		}else{
			return $this->db->get($table)->result_array();
		}
	}
	//get job Price
	public function get_jobvendorprice($job_id){
		$data = $this->db->select('sum(price) as  price')->where('job_id',$job_id)->get('job_prices')->row_array();
		if($data){
			if($data['price']){
				return $data['price'];
			}else{
	...
	public function get_ratingcount($vendor_id){
		return $this->db->select("COUNT(user_id) as usercount, AVG(rating)  as rating")->where('vendor_id',$vendor_id)->where("rating!=0")->get("jobs")->row_array();
	}
	//get vendor price by job id and service id
	public function get_vendorprice_byid($job_id,$service_id){
		return $this->db->where('job_id',$job_id)->where('service_id',$service_id)->get('job_prices')->row_array();
	}
	// completed jobs
